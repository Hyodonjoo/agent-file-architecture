...
def move_dir_contents(src_dir, dest_dir):
    ...
    shutil.rmtree(src_path)  # 소스 디렉토리 삭제

# 현재 실행되어 있는 프로그램을 종료하는 함수
def stop_program(program_name):
    try:
        subprocess.run(f'taskkill /F /IM {program_name}', check=True, shell=True)  # 프로세스 종료 명령 실행
...
def get_new_version_info(url):
    try:
        ...
        if response_data["ok"]:
            ...
            filenames = [fileInfo["filename"] for fileInfo in result["fileInfos"]]  # 파일 목록 추출
            if add_message_function:
                add_message_function(f"최신 버전: {result['version']}")  # 최신 버전 정보를 UI에 추가
...
def download_new_version(url, dest_dir):
    try:
        ...
        with open("new_version.zip", 'wb') as file:
            for chunk in response.iter_content(chunk_size=8192):  # 응답 데이터를 청크 단위로 받아서 저장
                file.write(chunk)
...
def replace_program(original_dir, new_version_dir, backup_dir):
    try:
        move_dir_contents(original_dir, backup_dir)  # 기존 파일을 백업 디렉토리로 이동
        move_dir_contents(new_version_dir, original_dir)  # 새로운 버전 파일을 원본 디렉토리로 이동
        if add_message_function:
...
def restore_backup(backup_dir, original_dir):
    try:
        move_dir_contents(backup_dir, original_dir)  # 백업 디렉토리에서 원본 디렉토리로 복구
        if add_message_function:
